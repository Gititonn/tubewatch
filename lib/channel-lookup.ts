...
import { calculateOutlierScores, channelMedianRate } from "@/lib/outlier";
...
export async function getChannelAnalytics(handle: string): Promise<ChannelAnalytics> {
  ...
  if (cache && Date.now() - new Date(cache.computed_at).getTime() < CACHE_TTL_MS) {
    const { data: rows } = await svc
      ...
      .select("youtube_video_id, title, thumbnail_url, published_at, view_count, like_count, comment_count, duration_seconds, outlier_score")
      .eq("youtube_channel_id", channelId)
      .limit(50);
  ...
  if (scored.length > 0) {
    ...
    await svc.from("video_view_history").insert(
      scored.map((v) => ({ youtube_video_id: v.youtube_video_id, view_count: v.view_count }))
    );
  }

  return { found: true, channel, videos: scored.sort(byScore) };
}
